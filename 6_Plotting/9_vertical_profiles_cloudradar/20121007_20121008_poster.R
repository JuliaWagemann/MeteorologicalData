...
pos1 <- (fogMain_current[1]+(0.3*(lengthIIa-25)))/rowNr
pos2 <- (fogMain_current[2]+(0.3*(lengthIIb-3)))/rowNr
pos3 <- (fogDiss_current[1]+(0.3*(lengthIII-20)))/rowNr
...
d = ggplot(mat.cr, aes(x=Hour_cr,y=Height_cr,fill=dBZ)) +
  annotate("rect", xmin=Hour_cr[fogMain_current[1]-32], xmax=Hour_cr[fogMain_current[2]-1],ymin=0.015,ymax=heightMax, alpha=.5,fill="lightgrey")+
 ...
 geom_segment(aes(x = Hour_cr[c33[4]], y = 0.025, xend = Hour_cr[c33[4]], 
              ...
              colour="#000000",size=0.5) +
 geom_segment(aes(x= Hour_cr[fogMain_current[1]-32], xend=Hour_cr[fogMain_current[2]-1],
