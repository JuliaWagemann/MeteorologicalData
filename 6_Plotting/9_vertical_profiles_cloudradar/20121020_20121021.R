...
source("vertProf_settings.R")

i <- "20121020_20121021_cloudinfo_dbz.csv"
fogEvent <- 8
...
c11 <- c(fogDev_current[3]+c1[1]-1,fogDev_current[3]+c1[2]-1,fogDev_current[3]+c1[3]-1)
...
IIa =grobTree(textGrob("I", x=0.03,hjust=0,y=0.99, vjust=1))
IIb = grobTree(textGrob("IIa", x=0.03, hjust=0,y=0.99,vjust=1))
III = grobTree(textGrob("IIb", x=0.03, hjust=0,y=0.99,vjust=1))
...
pos1 <- (fogDev_current[3]+(0.5*(lengthIIa-3)))/rowNr
pos2 <- (fogMain_current[1]+(0.3*(lengthIIb-3)))/rowNr
pos3 <- (fogMain_current[2]+(0.3*(lengthIII-12)))/rowNr

breaks_cr <- c(-50,-40,-30,-20,-10,0)
firstText =grobTree(textGrob("I", x=0.4,hjust=0,y=0.90, vjust=1))
secondText = grobTree(textGrob("IIa", x=pos2, hjust=0,y=0.90,vjust=1))
thirdText = grobTree(textGrob("IIb", x=pos3, hjust=0,y=0.90,vjust=1))
...
d = ggplot(mat.cr, aes(x=Hour_cr,y=Height_cr,fill=dBZ)) +
 ...
 geom_segment(aes(x= Hour_cr[fogDev_current[3]], xend=Hour_cr[fogDev_current[4]-1],
                  y= heightMax, yend=heightMax), colour="darkgrey", size=2) +
...
cGrob <- ggplotGrob(c)
dGrob <- ggplotGrob(d)
finalPlot <- grid.arrange(arrangeGrob(aGrob,bGrob,cGrob, ncol=3),dGrob, nrow=2,main=textGrob(title,gp=gpar(fontsize=20,font=1),vjust=1.5))
finalPlot
