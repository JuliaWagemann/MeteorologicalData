...
i <- "20120917_20120918_cloudinfo_dbz.csv"
fogEvent <- 2
...
lengthIIa <- fogDev_current[3] - fogDev_current[4]
lengthIIb <- fogMain_current[2] - fogMain_current[1]
lengthIII <- fogMain_current[3] - fogMain_current[2]

c1 <- c(35,39)
c2 <- c(5,12,20,28)
c3 <- c(0,0,0,0)

c11 <- c(fogDev_current[3]+c1[1]-1,fogDev_current[3]+c1[2]-1)
c22 <- c(fogMain_current[1]+c2[1]-1,fogMain_current[1]+c2[2]-1,fogMain_current[1]+c2[3]-1,fogMain_current[1]+c2[4]-1)
#c33 <- c(fogMain_current[2]+c3[1]-1,fogMain_current[2]+c3[2]-1,fogMain_current[2]+c3[3]-1,fogMain_current[2]+c3[4]-1)
c33 <- c(0,0,0,0)
####################################################################################
...
vertProf_21 <- df_cr5min[fogDev_current[3]:fogDev_current[4],]
vertProf_22 <- df_cr5min[fogMain_current[1]:fogMain_current[2],]
vertProf_23 <- df_cr5min[fogMain_current[2]:fogMain_current[3],]
...
IIa =grobTree(textGrob("I", x=0.03,hjust=0,y=0.98,gp=gpar(fontsize=8), vjust=1))
IIb = grobTree(textGrob("II", x=0.03, hjust=0,y=0.98,gp=gpar(fontsize=8),vjust=1))
III = grobTree(textGrob("III", x=0.03, hjust=0,y=0.98,gp=gpar(fontsize=8),vjust=1))
...
pos1 <- (fogDev_current[3]+(0.5*(lengthIIa-15)))/rowNr
pos2 <- (fogMain_current[1]+(0.3*(lengthIIb-3)))/rowNr
pos3 <- (fogMain_current[2]+(0.3*(lengthIII-18)))/rowNr

breaks_cr <- c(-50,-40,-30,-20,-10,0)
firstText =grobTree(textGrob("I", x=0.55,hjust=0,y=0.97,gp=gpar(fontsize=8), vjust=1))
secondText = grobTree(textGrob("II", x=pos2, hjust=0,y=0.97,gp=gpar(fontsize=8),vjust=1))
thirdText = grobTree(textGrob("III", x=pos3, hjust=0,y=0.97,gp=gpar(fontsize=8),vjust=1))


d = ggplot(mat.cr, aes(x=Hour_cr,y=Height_cr,fill=dBZ)) +
  annotate("rect", xmin=Hour_cr[fogDev_current[3]], xmax=Hour_cr[fogDev_current[4]-1],ymin=0.015,ymax=heightMax, alpha=.5,fill="lightgrey")+
  ...
  annotate("rect", xmin=Hour_cr[fogMain_current[2]], xmax=Hour_cr[fogMain_current[3]],ymin=0.015,ymax=heightMax, alpha=.5,fill="lightgrey")+
...
#               colour="#000000",size=1.1) +
 ...
 geom_segment(aes(x= Hour_cr[fogMain_current[1]], xend=Hour_cr[fogMain_current[2]-1],
                  y= heightMax, yend=heightMax), colour="darkgrey", size=1) +
 geom_segment(aes(x= Hour_cr[fogMain_current[2]], xend=Hour_cr[fogMain_current[3]],
                  y= heightMax, yend=heightMax), colour="darkgrey", size=1) +
...
finalPlot <- grid.arrange(arrangeGrob(aGrob,bGrob,cGrob, ncol=3),dGrob, nrow=2,main=textGrob(title,gp=gpar(fontsize=10,font=1,face="bold"),vjust=1.5))
dev.off()
