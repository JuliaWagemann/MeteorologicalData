...
fogEvent <- '20120930_20121001_Integrals.csv'
fogEventNr <- 4
...
rmse1_3 <- rmse(ZIntegral_ordered$dBZ_measured[breakPoints[1,fogEventNr]:TSLength],
                ZIntegral_ordered$dBZ_ground[breakPoints[1,fogEventNr]:TSLength])

rmse2_3 <- rmse(ZIntegral_ordered$dBZ_measured[breakPoints[1,fogEventNr]:TSLength],
                ZIntegral_ordered$dBZ_profile[breakPoints[1,fogEventNr]:TSLength])
...
###### For 2 Segments #############################################
segments(ZIntegral_ordered$datetime[1],1450,ZIntegral_ordered$datetime[breakPoints[1,fogEventNr]-1],
         1450, lwd=1.5, col="grey20")
segments(ZIntegral_ordered$datetime[breakPoints[1,fogEventNr]+1],1450,
         ZIntegral_ordered$datetime[length(ZIntegral_ordered$datetime)],1450,lwd=1.5,
         col="grey20")
...
mtext("II", side =3, line = -1.1,adj=0,at=ZIntegral_ordered$datetime[20])
mtext("III", side =3, line = -1.1,adj=0,at=ZIntegral_ordered$datetime[60])
...
legend("right",inset=c(-0.32,0),c('RMSE',
                                  ...
                                  paste('ground_II:',format(round(rmse1_2a,2),nsmall=2)),
                                  paste('profile_II:',format(round(rmse2_2a,2),nsmall=2)),
