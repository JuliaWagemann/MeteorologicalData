...
df_campbell <- data.frame(datetime = as.POSIXct(campbell$datetime), vvel=campbell$w,tp=campbell$tp,
                          tq=campbell$tq,tr=campbell$tr)
df_horVis <- data.frame(datetime = as.POSIXct(horVis$datetime), horVis=horVis$Vis)

tiff(file=paste(path,i,"._turb_poster_.tiff",sep=""),width=1382,height=1217, units="px",res=300, compression="lzw")
...
plot(df_campbell$datetime,df_campbell$vvel,type="l",
     ylim=c(-1,1),
     yaxp=c(-0.5,0.5,2),
     mgp=c(3,0.5,0))
mtext("[m " ~ s^-1 ~ "]",side=2,line=1.7, cex=0.8)
