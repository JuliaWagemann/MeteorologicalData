...
path <- "H:/Masterarbeit/2_Data/4_Plots/3_horVis_params/3_temp/"
...
df_campbell <- data.frame(datetime = as.POSIXct(campbell$datetime), Tair=campbell$T1_2m_Avg,
                          dp=D)
...
plot(df_campbell$datetime,df_campbell$dp,type="l",
     ylim=c(5,20),
     yaxp=c(0,20,4),
     lty=1,
     col="grey50",
     lwd=2,
     mgp=c(3,0.5,0),)
lines(df_campbell$datetime,df_campbell$Tair)
mtext("Time", side=1,line=1.7, cex=0.8)
mtext("[°C]",side=2,line=1.7, cex=0.8)
...
legend("topleft", c("Dew point", "Tair at 2m"),lty=c(1,1),col=c("grey50","black"),lwd=c(2,1),inset=0.01,
       cex=0.6,y.intersp=0.8,seg.len=2)
dev.off()
