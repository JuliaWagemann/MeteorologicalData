...
'''
###############################################################################
# Script 3_campbell_getFogEvents
# Author: Julia Wagemann
# Date: September 2013
#
# This script adds campbell files based on fog start and end values
###############################################################################
import glob
import csv
import os
...
def getFogEvents(inputfile1,inputfile2,fogStart,fogEnd):
    ...
    for line1, line2 in zip(file1,file2):
        ...
        colStr2 = ""
        colStr3 = ""
        # for fog events 19.12. has to  be tested, as this fog event started and ended on the
        # 19.12.
        if dateTime1.date() == test1:
            if dateTime1.time() >= fogStart1.time() and dateTime1.time() <= fogEnd1.time():
                ...
                colStr3=date3+','
                for i in range(3):
                    colStr3+=columns1[i+2]+','
                colStr3+=columns1[4]+'\n'
                outList3.append(colStr3)
        if len(outList3)!=0:
            ofile = open(inputfile1[:-13] + '_' + inputfile1[:-3] + '_campbell5min.csv', 'w')
        ...
        if dateTime2.time() <= fogEnd1.time() and dateTime2.date() == fogEnd1.date(): 
            ...
            colStr2 = date2 +','
            for i in range(3):
                colStr2+=columns2[i+2]+','
            colStr2+=columns2[4]
            outList2.append(colStr2)
    if len(outList1)!=0:
        ofile = open(inputfile1[:-13] + '_' + inputfile2[:-13] + '_campbell5min.csv', 'w')
        ofile.write('datetime,T1_2m_Avg,T2_10m_Avg,rH_2m,rH_10m\n')
